...
// 사진 찍는 함수
  const takePicture = async() =>{
    ...
    try{
      const response = await fetch('api로 연동할 여러분들의 주소를 여기에 적으면 됩니다. ^^/ocr/image', {
        method: "POST",
        body: formData,
        headers: {
...
// 인식 될때까지 버튼 안눌리도록 막기.
    ...
    if (response.ok) {
      const result = await response.json(); // 서버가 JSON 응답을 줄 경우에 진행.
  ...
  return (
    <View style={styles.container}>
      ...
      {isFocused ? (
        ...
        {/*하단의 buttonContainer와 button을 참고한다.*/}
